...
\centering
\begin{tabular}{lr}
\textbf{\textit{Time complexity}}        & \textbf{Big-$O$ notation}    \\
...
\noindent In the similar way as before it can be shown that the \textit{time complexity} for this problem is
$$T(n) \in O(3^n)$$
...
\noindent The Master theorem is a general method for solving (getting \textit{time complexity} for) recurrence relations that arise frequently in \textit{divide and conquer algorithms}, which have the following recurrence relation of the form: 
$$T(n) = a T\left(\dfrac{n}{b}\right)+f(n)$$
\noindent where $a \ge 1$, $b > 1$, and $f(n)$ is an asymptotically positive function. The \textit{time complexity} of such a recursive relation is given by the following rules:
...
\begin{center}
    ...
    \end{tikzpicture}
\end{center}
\caption{Deleting item from linked list}
\end{figure}
...
\begin{center}
    ...
    \end{tikzpicture}
\end{center}
\caption{Reversing a linked list: iterative approach}
\end{figure}
...
\begin{center}
    ...
    \end{tikzpicture}
\end{center}
\caption{Reversing a linked list: recursive approach}
\end{figure}
...
\subsection{Check if a linked list is palindrome or not (in constant \textit{space complexity})}
\vspace{5mm}
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \end{tikzpicture}
        \end{center}
        \caption{Check palindrome: Movement of \lstinline{slow} and \lstinline{fast} pointers for odd number of nodes} 
        \label{fig:Odd}
        \end{figure}
	...
	\begin{center}
        ...
        \end{tikzpicture}
        \end{center}
        \caption{Check palindrome: Movement of \lstinline{slow} and \lstinline{fast} pointers for even number of nodes} 
        \label{fig:Even}
        \end{figure}
...
% \noindent The value i.e. the phone number $9893260037$ corresponding to that key is stored in the index.  Ideal hashing should produce a different index for each key. However, \textbf{collisions} can occur whenever hash function produces the same index for multiple keys. The \textit{time complexity} in hashing is $O(1)$.
...
\noindent You can implement a tree structure by a pointer or an array. We choose to use the array implementation like below. In terms of \textit{space complexity}, the array implementation has more benefits than the pointer implementation. In the array representation of a heap, for an element in array index $i$,
\begin{itemize}
...
\noindent \textbf{\textit{Time complexity}}: The function \lstinline{heapify} exchanges two items in an array, which runs in constant time and recursively calls itself. So the \textit{time complexity} of \lstinline{heapify} will be in proportional to the number of repeats. In the worst case, \lstinline{heapify} should repeat the operation the height of the tree times. This is because in the worst case, \lstinline{heapify} will exchange the root nodes with the most depth leaf node. Therefore the \textit{time complexity} of \lstinline{heapify} is $O(\log_2 n)$ time. The function \lstinline{buildHeap} basically calls \lstinline{heapify} and makes $O(n)$ such calls. So the overall \textit{time complexity} is $O(n\log_2 n)$.
...
\begin{enumerate}
    \item Build a max-heap from the elements of the array \lstinline{arr}.
    \item The root element, that is \lstinline{arr[0]}, will contain maximum element of \lstinline{arr}. Swap this element with the last element of \lstinline{arr}. The last element is already in its correct position.
    \item Decrease the length of heap by one which will exclude the last element.
    \item Heapify the root element (bubble-down operation of root) again so that we have the highest element moves at the root again.
    \item The process is repeated until all the items of the list are sorted.
\end{enumerate}
...
\begin{figure}[H]
	...
	every node/.style = {minimum width = 2em, draw, rectangle, node distance={2em}}]
	\node (A) {12};
	\node[draw=none, fill=white!20] (A1) [above=.1cm of A] {0};
	\node (B) [right of=A] {6};
	\node[draw=none, fill=white!20] (B1) [above=.1cm of B] {1};
	\node (C) [right of=B] {10};
	\node[draw=none, fill=white!20] (C1) [above=.1cm of C] {2};
	\node (D) [right of=C] {5};
	\node[draw=none, fill=white!20] (D1) [above=.1cm of D] {3};
	\node (E) [right of=D] {1};
	\node[draw=none, fill=white!20] (E1) [above=.1cm of E] {4};
	\node (F) [right of=E] {9};
	\node[draw=none, fill=white!20] (F1) [above=.1cm of F] {5};
	\end{tikzpicture}
	\caption{Heap sort: Step 1. Initialisation}
\end{figure}
...
\begin{figure}[H]
	...
	\node (E) [right of=D] {1};
	\node[draw=none, fill=white!20] (E1) [above=.1cm of E] {4};
	\node[fill=gray!30] (F) [right of=E] {12};
	\node[draw=none, fill=white!20] (F1) [above=.1cm of F] {5};
	\end{tikzpicture}
	\caption{Heap sort: Step 2. Swapping 12 with 9 and remove 12 from heap}
\end{figure}
...
\begin{figure}[H]
	...
	\node (D) [right of=C] {5};
	\node[draw=none, fill=white!20] (D1) [above=.1cm of D] {3};
	\node[fill=gray!30] (E) [right of=D] {10};
	\node[draw=none, fill=white!20] (E1) [above=.1cm of E] {4};
	\node[fill=gray!30] (F) [right of=E] {12};
	\node[draw=none, fill=white!20] (F1) [above=.1cm of F] {5};
	\end{tikzpicture}
	\caption{Heap sort: Step 3. Bubble-down 9}
\end{figure}
...
\begin{figure}[H]
	...
	\node[draw=none, fill=white!20] (F1) [above=.1cm of F] {5};
	\end{tikzpicture}
	\caption{Heap sort: Step 4. Swapping 10 with 1 and remove 10 from heap}
\end{figure}
...
\begin{figure}[H]
	...
	\node (C) [right of=B] {1};
	\node[draw=none, fill=white!20] (C1) [above=.1cm of C] {2};
	\node[fill=gray!30] (D) [right of=C] {9};
	\node[draw=none, fill=white!20] (D1) [above=.1cm of D] {3};
	\node[fill=gray!30] (E) [right of=D] {10};
	\node[draw=none, fill=white!20] (E1) [above=.1cm of E] {4};
	\node[fill=gray!30] (F) [right of=E] {12};
	\node[draw=none, fill=white!20] (F1) [above=.1cm of F] {5};
	\end{tikzpicture}
	\caption{Heap sort: Step 5. Bubble-down 1}
\end{figure}
...
\begin{figure}[H]
	...
	\node[draw=none, fill=white!20] (F1) [above=.1cm of F] {5};
	\end{tikzpicture}
	\caption{Heap sort: Step 6. Swap 9 with 5 and remove 9 from heap}
\end{figure}
...
\begin{figure}[H]
	...
	\node (B) [right of=A] {5};
	\node[draw=none, fill=white!20] (B1) [above=.1cm of B] {1};
	\node[fill=gray!30] (C) [right of=B] {6};
	\node[draw=none, fill=white!20] (C1) [above=.1cm of C] {2};
	\node[fill=gray!30] (D) [right of=C] {9};
	\node[draw=none, fill=white!20] (D1) [above=.1cm of D] {3};
	\node[fill=gray!30] (E) [right of=D] {10};
	...
	\node[draw=none, fill=white!20] (F1) [above=.1cm of F] {5};
	\end{tikzpicture}
	\caption{Heap sort: Step 7. Bubble-down 5}
\end{figure}
...
\begin{figure}[H]	
	...
	\node[draw=none, fill=white!20] (F1) [above=.1cm of F] {5};
	\end{tikzpicture}
	\caption{Heap sort: Step 8. Swap 6 with 1 and remove 6 from heap}
\end{figure}
\begin{figure}[H]	
	\centering
	\begin{tikzpicture}[
	...
	\node[draw=none, fill=white!20] (F1) [above=.1cm of F] {5};
	\end{tikzpicture}
	\caption{Heap sort: Step 9. Bubble down 1}
\end{figure}
\begin{figure}[H]
	\centering	
	...
	\node (A) {1};
	\node[draw=none, fill=white!20] (A1) [above=.1cm of A] {0};
	\node[fill=gray!30] (B) [right of=A] {5};
	\node[draw=none, fill=white!20] (B1) [above=.1cm of B] {1};
	\node[fill=gray!30] (C) [right of=B] {6};
	\node[draw=none, fill=white!20] (C1) [above=.1cm of C] {2};
	\node[fill=gray!30] (D) [right of=C] {9};
	...
	\node[draw=none, fill=white!20] (F1) [above=.1cm of F] {5};
	\end{tikzpicture}
	\caption{Heap sort: Step 10. Swap 5 with 1 and remove 5 from heap}
\end{figure}

\noindent \textit{Time complexity}: The initial building of heap takes $O(n\log n)$ time. The the bubbline-down operation of root would take $O(\log n)$ time, which is to be performed $n$ times at maximum. So the \textit{time complexity} for heap sort is $O(n\log n)$.\\
\noindent \textit{Space complexity}: $O(1)$
\begin{lstlisting}
...
%\noindent A disjoint set is a data structure which is a set of elements partitioned into several disjoint (non-overlapping) subsets. In other words, a disjoint set is a group of sets where no item can be in more than one set. It is also called a \textit{union-find} data structure as it supports union and find operation on subsets. It supports the following operations:
%
%\begin{enumerate}
%	\item \textbf{makeSet}($x$): This operation creates a new set containing element $x$. A set with only one element is called a singleton set.
%	\item \textbf{find}($x$): This operation returns the representative of the set in which the element $x$ is present.
%	\item \textbf{union}($x$, $y$): This creates a new set containing the elements of sets $x$ and $y$ and then deletes the individual sets $x$ and $y$.
%\end{enumerate}
%
%\begin{lstlisting}
...
\noindent If $m$ is the number of neighbors of a given vertex, and $v$ is the number of vertices in the graph, then the \textit{time complexity} comparison for different representations are given in the following table.
\begin{table}[ht]
\centering 
\begin{tabular}{lccc}
...
\end{tabular}
\caption{\textit{Time complexity} comparison}
\end{table}
...
3               8
\end{lstlisting}
\noindent \textit{Time complexity}: The \textit{time complexity} of the algorithm is $O(|V||E|)$, because the algorithm consists of
...
[20, 1, 1, 1]
\end{lstlisting}
\noindent We can easily see that the algorithm is not going to take more than linear time. As $n$ is decreased by \lstinline{denominations[i]} at the end of the while loop, we can say that for most of the cases it will take much less than $O(n)$ time. So, we can say that our algorithm has $O(n)$ \textit{time complexity}.
...
\noindent A simple solution is to generate all subsets of given set of jobs and check individual subset for feasibility of jobs in that subset. Keep track of maximum profit among all feasible subsets. The \textit{time complexity} of this solution is exponential. We can also solve this problem using greedy method which is more optimal than simple solution. So let's solve this problem via greedy method. 
...
\vspace{5mm}
\noindent \textit{Time complexity}: First we need to sort the items in decreasing order of value/weight ratio, which takes $O(n\log n)$ time. Then iteration over each element takes $O(n)$ time. So the \textit{time complexity} is $O(n\log n)$.\\
...
\begin{enumerate}
    \item \textit{Decision Problem}: In this type of problem we always search for a feasible solution.
    \item \textit{Optimization Problem}: In this type of problem we always search for the best possible solution.
    \item \textit{Enumeration Problem}: In this type of problem we try to find all feasible solutions.
\end{enumerate}
...
\noindent Here, the program will call itself, again and again, to calculate further values as shown in the figure. The calculation of the \textit{time complexity} of the recursion based approach is around $O(2^n)$. The \textit{space complexity} of this approach is $O(n)$ as recursion can go max to $n$. In this method values like $F_2$ and $F_3$ are computed multiple times. Imagine the number of repetitions if you have to calculate for $F_{100}$. This method is ineffective for large values.
...
\noindent A naive solution is to check if every subsequence of $X$ to see if it is also a subsequence of $Y$. As there are $2^m$ subsequences possible of $X$, the \textit{time complexity} of this solution would be $O(n.2^m)$, where $m$ is the length of the first string and $n$ is the length of the second string.
...
\noindent \textit{Time complexity}: $O(nm)$: Since there are $(m+1)(n+1)$ entries, so the total number of recursive calls are at most $2(m+1)(n+1)+1$. So the \textit{time complexity} is $O(mn)$.\\
...
\noindent Second solution: using \textit{two pointer technique} (in in constant \textit{space complexity})
\begin{enumerate}
...
%\noindent Second approach: We know that an \textit{inorder traversal} of a binary search tree returns the nodes in sorted order. To determine whether a given binary tree is a BST, keep track of the last visited node while traversing the tree. Then for each encountered node in the inorder traversal, check whether the last visited node is smaller compared to the current node.
%
%\vspace{4mm}
%
%\noindent \textit{Time complexity}: $O(n)$ where $n$ is the size of the tree.\\
%\noindent \textit{Space complexity}: $O(n)$ since there will be $n$ number of recursive calls.\textbf{}
%
%\begin{lstlisting}
%class Tree_node:
%    def __init__(self, data, left = None, right = None):
...
%    prev.data = node.data
%
%    # Check if the right subtree is BST or not
%    isBST(node.right, prev)
% 
% 
%# Function to determine whether a given binary tree is a BST
%def checkForBST(root):
%    # Pointer to store previously processed node in the inorder traversal
%    prev = Tree_node(float("-Inf"))
%    if isBST(root, prev):
%        print('The tree is a BST.')
%    else:
%        print('The tree is not a BST')
